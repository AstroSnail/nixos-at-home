...
let
  ...
  mkAccountHash = data: mkHash "${builtins.toString data.server} ${data.keyType} ${data.email}";
  ...
  account-configs =
    certs:
    lib.groupBy' (certs: conf: certs ++ [ conf.cert ]) [ ] (conf: conf.accountHash) (certConfigs certs);

in
{
  lib.acme = { inherit account-configs; };
}
