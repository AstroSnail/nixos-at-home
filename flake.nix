    ...
    let
      ...
      flakeOutputs =
        ...
        let
          f-with-system =
            system:
            lib.mapAttrsToList (name: value: lib.nameValuePair name (lib.nameValuePair system value)) (
              f system
            );
          f-all-systems = lib.concatMap f-with-system supportedSystems;
          combined-outputs = lib.groupBy (x: x.name) f-all-systems;
          combine-systems = output-systems: lib.listToAttrs (lib.lists.map (x: x.value) output-systems);
        in
        lib.mapAttrs (name: combine-systems) combined-outputs;

      concatGenAttrs =
        names: f: lib.listToAttrs (lib.concatMap (n: lib.mapAttrsToList lib.nameValuePair (f n)) names);
      ...
      nixosModules = forAllServices (name: {
        imports = [
          "${self}/main.nix"
          "${self}/${name}"
        ];
      ...
      nixosConfigurations = forAllServicesFlat (
        name:
        let
        in
        { }
      ...
      let
        ...
        packages = forAllServicesFlat (
          ...
          let
            ...
            control = pkgs.writeText "control-${name}" nixos.config.debianControl;
            ...
            postinst =
              let
                script = nixos.config.postInstallScript;
              in
              if script != "" then pkgs.writeText "postinst-${name}" script else null;
